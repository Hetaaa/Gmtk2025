...
func _ready():
	...
	if FightManager.enemy_ref:
		FightManager.enemy_ref.health_changed.connect(_on_health_changed)
...
func update_health_bar():
	if not FightManager or not FightManager.enemy_ref:
		return
	
	var current_hp = FightManager.enemy_ref.current_health
	var max_hp = FightManager.enemy_ref.max_health
